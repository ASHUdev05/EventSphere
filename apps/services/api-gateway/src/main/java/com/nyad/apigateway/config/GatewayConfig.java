package com.nyad.apigateway.config;
  ...
  RouterFunction<ServerResponse> customRoutes() {
    return route("auth-manager")
      ...
      .and(route("event-manager")
        ...
        .build())
      .and(route("notification-manager")
        .route(path("/api/v1/notification-manager/**"), http())
        .before(stripPrefix(3))
        .filter(lb("NOTIFICATION-MANAGER"))
        .filter(circuitBreaker("notification-manager-cb", URI.create("forward:/fallback/notification-manager")))
        .build())
